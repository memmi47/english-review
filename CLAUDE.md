...
4. **3가지 문제 유형으로 변환**:
   ...
   - **pattern**: 같은 화석화 패턴(예: `succeed to` vs `succeed in -ing`)을 **완전히 새로운 문장/맥락**으로 3~5개 만들어 `group_id`로 묶음 — 그날 대화를 기억 못 해도 풀 수 있어야 진짜 실력 테스트임. 절대 사용자가 실제로 말했던 문장을 그대로 빈칸화하지 말 것 (기억력 테스트가 됨)
